...
**Landing Page:** 2 centered text input boxes labeled “Team \#1 Name” and “Team \#2 Name”. A button under these fields labeled “Start Game”. When clicked the page transitions smoothly into the view of the full board. Below the button, a “How to play” link opens a scrollable overlay that is the game's full manual, so players never need the README to learn the game. It covers the mission (why the math theme, the anti-fear intent), who plays (teams, host, that 3-4 is recommended but the game scales, calculators), the board, the three acts, taking turns, wrong answers, scoring, and the host's job (judging plus keeping the room constructive, and the optional host console and what it does). Keep this overlay in sync with README's “How to play” section; the two are the player-facing manual and must agree. The overlay closes via its “x”, a click on the backdrop, or Escape. The landing page also carries the dark-mode toggle (top right) and, when host sync is configured, the host-console pairing chip.
