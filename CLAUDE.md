...
- **Console rendering scope.** The console renders only Board Content, Scoring, and Host Notes fields, each visually grouped with a left accent and a source tag (Board Content neutral, Scoring green, Host Notes gold). Documentation fields (why it's this level, build, reskin, and other notes) are author-facing: they stay in the content files and never render anywhere in the game.
...
- **Integration points.** sync.js wraps State.save to publish persisted changes; card.js calls a ping() for UI-only changes (open, flip, close, timer). Commands are applied through the same public APIs the on-screen controls use (Card.open, Card.flipToAnswer, Card.toggleWinner, Card.toggleTimer, Card.close), with Board.isOpenable guarding remote opens by the same rule as tile clicks. If a new mechanic adds state, make sure it flows through State.save or pings sync.
