...
- **Purpose.** The TV board stays player-clean (an invariant); hints, host notes, and takeaways render only on the host's phone via host.html. The console also remote-controls the board: open/close cards, flip, winner selection, timer (its controls sit beside the timer readout in the question view). Opening a question on the TV is deliberately two-step: tapping a square always shows a private preview (including locked rows, so the host can read ahead), and an explicit "Open on TV" button in the preview is what sends it to the shared screen. Nothing on the mini board reaches the TV in one tap; accidental reveals are structurally impossible.
...
- **Remote controls.** Beyond opening/flipping/winners/timer, the console header carries Dark mode and Unlock acts toggles so it works as a full remote. These send `dark` and `override` commands; the TV applies them via Main.setDark / Main.setOverride and rebroadcasts. Dark state travels in the snapshot (`dark`) so the console and TV stay matched; the `dark` command is allowed on any screen (it sits ahead of the started/finished guard in applyCommand), `override` only during play.
