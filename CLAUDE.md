...
**Landing Page:** 2 centered text input boxes labeled “Team \#1 Name” and “Team \#2 Name”. A button under these fields labeled “Start Game”. When clicked the page transitions smoothly into the view of the full board. Below the button, a “How to play” link opens a scrollable overlay: the in-game manual, so players never need the README to learn the game. It is deliberately tight, state the goal once in the opening line, then give thorough but terse rules with no design justification (the reasoning behind choices lives in README, not here). It must cover every game element: who plays (teams, host, that 3-4 is recommended but any size works, calculators), the board (columns/rows/points, hidden squares), the three acts, taking turns, wrong answers and the steal, scoring, the host's job, and the optional host console. The overlay and README's “How to play” must agree on the rules but not match in depth: README may explain why, the overlay only says what. The overlay closes via its “x”, a click on the backdrop, or Escape. The overlay markup and behavior live in one shared module (js/rules.js + css/rules.css) used by both the board and the host console, so the rules text has a single home; any element with class `rules-open` opens it. The landing page also carries the dark-mode toggle (top right) and, when host sync is configured, the host-console pairing chip.
...
- **Dark Mode**: Toggles labeled "Dark mode" on both the landing page (top right corner) and the board (next to the unlock toggle, kept in step) switch the whole page to a dark theme; the warm palette and column colors hold. Card faces darken too (near black against the dark stone background), and in light mode they sit a step darker than the page background rather than brighter, since the card fills the screen for most of the game and must not glare. Charts re-render with a theme-matched palette (js/charts.js applyTheme) and tables ride on the CSS variables, so content stays legible in both themes. The preference is a display setting, not game state: it persists in its own localStorage key and survives "Restart game". Default is off (light).  
- **Tracking Points**:   
...
- **Card contents:** Cards are player-clean. The front face shows the question (plus its visual) and the back face shows the answer (plus the corrected visual where one exists). Takeaways and everything in the Host Notes section (hints, host notes) are not rendered; the host reads them from another screen: the host console (host.html, see below) or a printout of the content files. 

**Host Console (host.html) and sync:**

- **Purpose.** The TV board stays player-clean (an invariant); hints, host notes, and takeaways render only on the host's phone via host.html. The console also remote-controls the board: open/close cards, flip, winner selection, timer (its controls sit beside the timer readout in the question view).
- **Starting the game from the console.** When connected before the game starts, the console shows a start form (team names, Start Game, How to play) mirroring the board's landing page. Starting sends a `start` command with the team names; the TV applies it via Main.startGame and transitions to the board. This is the same capability the board's landing page has, available remotely.
- **Connected vs disconnected.** By default the console is connected: it mirrors the TV, and tapping a square opens that question on the shared screen directly (the host already sees every answer on the console, so there is no reveal to hide, and it is understood that the point and Question/Answer controls drive the board). A "Disconnect from players" toggle on the console's board view lets the host peek privately: it auto-unlocks all acts locally (reverting on reconnect, the TV's own override is never touched), lets any question be opened to read without affecting the TV, and shows the point and timer controls but disabled, so nothing changes on the shared screen without players understanding why. The host returns to the board view to reconnect before resuming shared control.
- **Console rendering scope.** Opening a question folds its fields into three collapsible groups matching the Markdown source sections: Board content (the question and the answer), Scoring (what earns the points, how they might get there), and Host notes (hints, host note, takeaway), color-accented neutral/green/gold. This mirrors the content-file grouping; do not split each field into its own container. Documentation fields (why it's this level, build, reskin, and other notes) are author-facing: they stay in the content files and never render anywhere in the game.
- **Console layout and theming.** The console's board view mirrors the TV board's grid proportions (row-label column plus four equal columns, column headers, act bands), centered with a max width, so it reads correctly on a phone or a desktop rather than a squished phone column. A "Host Console" pill marks the screen as the control surface, not the board. The console supports dark mode (its own tokens); in live mode it mirrors the TV's theme, when browsing it follows a local preference.
- **Remote controls and toggles.** All three session toggles are grouped in the console header (Disconnect from players, Unlock all acts, Dark mode); labels match the board and are not uppercased. Dark is always shown; unlock and disconnect only in live mode. When connected, the Unlock all acts toggle sends an `override` command; the Dark mode toggle sends a `dark` command; the TV applies them via Main.setOverride / Main.setDark and rebroadcasts. Dark state travels in the snapshot (`dark`) so the console and TV stay matched; the `dark` command is allowed on any screen (it sits ahead of the started/finished guard in applyCommand), `override` only during play. When disconnected, Unlock all acts is a local view setting and sends nothing.
- **Optimistic controls.** Console actions that drive the TV (open, close/back, flip, winner) update the console immediately from a local pending overlay (effOpenQid/effFace/effWinners in host.js), then reconcile when the next snapshot lands (all pending cleared on snapshot). Without this the buttons would only respond after a full Firebase round-trip, which feels laggy on a weak connection. The delay itself is inherent to remote control (console to cloud to TV to cloud to console); the overlay just hides it.
- **Team point totals placement.** The score totals show on the console's board view (as on the player board's corners, with progress bars), never on the card or in the header. Because they are only visible on the board view, a point change appears to the host only after the card is closed, matching the player view where the total updates after close. Winner selection stays on the card (as on the player view); the "TV: Question/Answer" face chip was removed.
- **Authority model.** The TV (index.html) is authoritative and fully standalone. js/sync.js is an additive layer: when js/firebase-config.js holds a Firebase config, the TV publishes a state snapshot to Firebase Realtime Database under rooms/{code}/state on every change, and consumes commands the phone pushes to rooms/{code}/cmd. With config null, or no internet, every sync entry point is a no-op and the game behaves exactly as before. Never let game logic depend on the sync layer.
- **Integration points.** sync.js wraps State.save to publish persisted changes; card.js calls a ping() for UI-only changes (open, flip, close, timer); Main.setDark calls Sync.notify() so theme changes broadcast too. Commands are applied through the same public APIs the on-screen controls use (Card.open, Card.flipToAnswer, Card.toggleWinner, Card.toggleTimer, Card.close, Main.setOverride, Main.setDark), with Board.isOpenable guarding remote opens by the same rule as tile clicks. If a new mechanic adds state, make sure it flows through State.save or pings sync.
- **Timer sync.** The TV publishes {total, remaining, running, at} only on start/pause/reset, not per tick; the phone computes the countdown locally from the timestamp.
- **Phone fallback.** host.html has a browse mode with no connection: the same mini board and note views, navigated by hand. It is the automatic degradation path and also works with sync unconfigured.
- **Room codes.** Four letters, persisted per TV browser in localStorage, shown as a "Host console" chip on both the landing page and the board footer, opening a QR pairing overlay (vendored js/vendor/qrcode.min.js). Database rules expose only the rooms branch; anyone with a code can control that room, an accepted trade-off for a party game. Setup steps and the rules JSON live in the header comment of js/firebase-config.js; README describes the capability without the setup.
- **Presentation coupling note applies to the console too.** host.js renders questions through the same Charts and Format modules as the TV card, so a re-theme that touches charts.js or format.js affects both screens.
