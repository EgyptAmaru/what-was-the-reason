...
- **Remote controls and toggles.** Dark mode lives in the console header (reachable from any view); Unlock all acts and Disconnect from players live on the console's board view. Labels match the board and are not uppercased. When connected, the Unlock all acts toggle sends an `override` command; the Dark mode toggle sends a `dark` command; the TV applies them via Main.setOverride / Main.setDark and rebroadcasts. Dark state travels in the snapshot (`dark`) so the console and TV stay matched; the `dark` command is allowed on any screen (it sits ahead of the started/finished guard in applyCommand), `override` only during play. When disconnected, Unlock all acts is a local view setting and sends nothing.
