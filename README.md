...
* **Designed for the real viewing context**: One shared screen, host and players together, legible from across a room on a TV. The muted palette, large tile labels, and player-clean cards (host material stays on the host's phone or a printout) follow from how the game is actually played, not from how it looks in a mockup. An optional dark mode (toggleable from the landing page or the board) darkens everything, cards included, for dim rooms; in light mode the cards sit a shade darker than the page so the screen never glares, and charts adapt their palette to the theme.
